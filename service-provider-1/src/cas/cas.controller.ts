...
export class CasController {
	...
	private SP_cas_service = 'http://127.0.0.1:3001/cas';
	...
	async authenticate(
		...
		try {
			...
			if (isSuccessed === true) {
				...
				res.cookie('username-sp1', parsedResult['cas:serviceResponse']['cas:authenticationSuccess'][0]['cas:user'], { httpOnly: false });
	...
	getLogoutPage(@Res() res: Response, @Req() req: Request) {
		return { message: req.cookies["username-sp1"], method: 'cas'};
	}
