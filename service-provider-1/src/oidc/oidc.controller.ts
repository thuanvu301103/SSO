...
export class OidcController {
	...
	getLoginPage(
		...
		let redirect_link = this.IdP_oidc_loginpage + '?response_type=code&client_id=sp1&redirect_uri=http://127.0.0.1:3001/oidc/callback&scope=openid&state=random_state_value';
		...
		@Req() req: Request)
	{
		...
		try {
			...
			res.cookie('username-sp1', des_id_token['username'], { httpOnly: false });
	...
	*/
	...
	getLogoutPage(@Res() res: Response, @Req() req: Request) {
		return { message: req.cookies["username-sp1"], method: 'oidc'};
	}
	...
	logOut(@Body('logout') logout: string, @Res() res: Response, @Req() req: Request) {
		...
		else res.redirect('/oidc/dashboard');
	}
